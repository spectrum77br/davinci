...
type Page = { items: MargemAudit[]; total: number; limit: number; offset: number }

const { api } = useApi()

const items = ref<MargemAudit[]>([])
const total = ref(0)
const limit = ref(50)
const offset = ref(0)
...
function acaoPill(a: string) {
  if (a === 'situacao') return 'pill pill-success'
  if (a === 'saldo_final') return 'pill pill-warning'
  return 'pill pill-muted'
}

function quem(row: MargemAudit) {
...
async function load() {
  ...
  try {
    ...
    if (filtroAcao.value) qs.set('acao', filtroAcao.value)
    if (filtroOrigem.value) qs.set('origem', filtroOrigem.value)
    const r = await api<Page>(`/api/margem-audit?${qs}`)
    items.value = r.items
    total.value = r.total
  } catch (e: any) {
...
<template>
  <div class="space-y-5">
    ...
    <div class="flex flex-wrap gap-2 items-center">
      ...
      <select v-model="filtroAcao" class="h-9 rounded-md border bg-background px-2 text-sm" @change="refreshAll">
      ...
      <select v-model="filtroOrigem" class="h-9 rounded-md border bg-background px-2 text-sm" @change="refreshAll">
        ...
        <option value="margens">Margem</option>
      ...
      <table class="w-full">
        ...
        <tbody>
          ...
          <tr v-else-if="items.length === 0">
            <td colspan="7" class="text-center text-muted-foreground py-6">sem registros</td>
          </tr>

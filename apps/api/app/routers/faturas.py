...
async def create_fatura(
    ...
    f = Fatura(
        ...
        detalhes=body.detalhes.strip() if body.detalhes and body.detalhes.strip() else None,
...
async def patch_fatura(
    ...
    if "detalhes" in data:
        f.detalhes = data["detalhes"].strip() if data["detalhes"] and data["detalhes"].strip() else None
    if "plano" in data:

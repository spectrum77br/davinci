...
async def get_video_map(
    ...
    """Returns `{pricing_product_id: has_video}` for the grid's video filter.

    A PricingProduct is "with video" if ANY ML Listing sharing its SKU
    (case-insensitive) has a non-empty `raw_data->>'video_id'`. ML stores
    `video_id` at the item root when the seller attached a YouTube video;
    absent/null/empty string all mean "no video".

    Match is by SKU only (not by `product_id` ↔ ProductLink) because:
      1. PricingProduct.sku is always populated; PricingProduct.product_id
         is sparse in prod (~half the rows).
      2. PricingProduct.sku is the same thing the operator types into the
         grid search, so the mental model "the product I see has video"
         maps directly onto the join.

    Compound SKUs (`a,b,c` in PricingProduct.sku) are split — the
    pricing_product is "with video" if AT LEAST ONE piece's listings has
    a video. That matches how operators think about kits: as long as one
    component item has the asset, the kit can use it.
    """
    _, leaves_by_id, root_id_by_slug = await _segment_index(session)
    ...
    # Collect every SKU piece referenced by these pricing_products so we
    # only fetch listings we actually need (some users have 20k+ listings).
    ...
    for pp in pricing_rows:
        for piece in (pp.sku or "").split(","):
            ...
            if k:
                needed_skus.add(k)
    if not needed_skus:
        return {str(pp.id): False for pp in pricing_rows}

    # `raw_data->>'video_id'` is the JSON text accessor; NULL handles
    # both missing key and explicit null. The empty-string compare
    # catches sellers who attached an empty video_id field.
    listing_rows = (
        await session.execute(
            select(Listing.sku).where(
                and_(
                    Listing.user_id == user.id,
                    Listing.platform == IntegrationPlatform.ML,
                    Listing.sku.isnot(None),
                    func.lower(func.trim(Listing.sku)).in_(needed_skus),
                    Listing.raw_data["video_id"].astext.isnot(None),
                    Listing.raw_data["video_id"].astext != "",
                )
            )
        )
    ).scalars().all()
    skus_with_video = {(s or "").strip().lower() for s in listing_rows}

    out: dict[str, bool] = {}
    for pp in pricing_rows:
        pieces = [p.strip().lower() for p in (pp.sku or "").split(",") if p.strip()]
        out[str(pp.id)] = any(piece in skus_with_video for piece in pieces)
    ...
    logger.info(
        ...
        department=department,
        pricing_products=len(pricing_rows),
        skus_needed=len(needed_skus),
        skus_with_video=len(skus_with_video),
        with_video=sum(1 for v in out.values() if v),
    )
    return out

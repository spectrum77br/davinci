...
@pytest.mark.asyncio
async def test_lotes_lista_com_faturador_e_qtd(
    db: AsyncSession, client: AsyncClient, admin: User, auth_as: Callable[[User | None], None]
):
    auth_as(admin)
    await _seed_dois_faturadores(db, admin)
    ...
    r = await client.get("/api/nf-cadastro/faturamento/lotes")
    assert r.status_code == 200, r.text
    lotes = r.json()
    assert len(lotes) == 2
    por_fat = {l["faturador_nome"]: l for l in lotes}
    assert set(por_fat) == {"bling avulso", "bling exclusivo"}
    assert por_fat["bling avulso"]["qtd_pedidos"] == 1
    assert por_fat["bling avulso"]["faturador_modo"] == "bling"
    assert all(l["status"] == "pending" for l in lotes)
    assert all(l["action"] == "import_avulsa" for l in lotes)


@pytest.mark.asyncio
async def test_lote_excel_agrega_por_sku(
    db: AsyncSession, client: AsyncClient, admin: User, auth_as: Callable[[User | None], None]
):
    auth_as(admin)
    await _seed_dois_faturadores(db, admin)
    # segundo pedido do mesmo faturador/loja, MESMO sku → agrega
    await _seed_pedido(db, numero="830003", loja="930001", sku="dg053.ci",
                       nome="Capa", unit=500)
    await db.commit()
    await client.post("/api/nf-cadastro/faturamento/enfileirar",
                      json={"numeros": ["830001", "830003"]})

    lotes = (await client.get("/api/nf-cadastro/faturamento/lotes")).json()
    lote_id = next(l["id"] for l in lotes if l["faturador_nome"] == "bling avulso")

    r = await client.get(f"/api/nf-cadastro/faturamento/lotes/{lote_id}/excel")
    assert r.status_code == 200, r.text
...
@pytest.mark.asyncio
async def test_lote_excel_404(
    db: AsyncSession, client: AsyncClient, admin: User, auth_as: Callable[[User | None], None]
):
    auth_as(admin)
    r = await client.get(f"/api/nf-cadastro/faturamento/lotes/{uuid.uuid4()}/excel")
    assert r.status_code == 404
    assert r.json()["detail"]["code"] == "nf_command_not_found"


@pytest.mark.asyncio
async def test_lotes_exige_auth(db: AsyncSession, client: AsyncClient):
    r = await client.get("/api/nf-cadastro/faturamento/lotes")
    assert r.status_code == 401
